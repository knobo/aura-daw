...
npm test                      # 834 frontend tests (828 unit + 6 DOM-mounted; counted 2026-08-18 after the read-only version-history browser; vitest): stores + timeline math + section-table bijection + library store + automation/modulation edit ops + MIDI I/O + group drag/resize + clip selection/clipboard + frontend clipboard codec/orchestration + MIDI selection export + piano-roll quantize + recent-projects + tempo editor + launch map + pitch wire types + pitch frame bus + pitch lane geometry + pitch panel logic + pitch subscription lifetime + rehearse-hold refcount + pitch report geometry + lane layout/arrange + the composer store + circle geometry + the palette colour ramp + AutomationTrackRow pointer/gesture handlers + read-only version-history store + mounted History panel
