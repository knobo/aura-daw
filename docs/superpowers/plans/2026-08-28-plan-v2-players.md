...
| **V-17** | **A player is not PDC-padded, and where its dry path is compensated depends on where it is ROUTED.** The source path (`bus::compile_routing`'s `track_pdc`) is forced to 0 for `MixNodeKind::Player`, always. The dry path (`out_delay`) is 0 for a player routed to the MASTER, and `t_in[bus] - ready` — the ordinary track expression, with `ready` the player's own applied chain latency — for a player routed INTO A BUS. Delay compensation aligns material that shares a TIMELINE POSITION, and a player has none: its row reads its own clock, and its reference for "on time" is the performer's hand and the arrangement audio already leaving the speakers. Padding it by the graph maximum puts the pad *after* the beat it was aimed at: 64–512 samples (1.3–11 ms) with one oversampled saturator anywhere in the session, and 2048–4096 samples (43–85 ms) with one linear-phase EQ — two to four times past the point where a triggered hit stops feeling attached to the press. That argument governs the master, which is the default and the path the owner's ear-check uses. Inside a BUS it does not: a bus is a summing point, everything entering it must arrive together or the mix smears, and the pad's send edge into that bus is compensated already (see (a)). Zeroing only the dry half put a pad's dry `t_in` samples ahead of its own send copy at the same summing point — the pad smeared against ITSELF. The accepted consequence: a pad routed into a bus that contains a linear-phase-EQ'd track inherits that latency. That is the price of an unsmeared bus, it is opt-in through the routing, and master stays at zero. Two consequences follow and both are load-bearing. (a) SEND-EDGE compensation (`RtSend::delay`) stays: a pad's send into a shared bus must still arrive in step with that bus's other inputs, which is a different question from where the pad's own dry signal belongs in time. Because the row is no longer padded up to `t_src`, the edge delay is measured from the player's OWN applied chain latency instead — leaving it at `t_src` would make a pad's send arrive `t_src` samples early and break the one alignment this ruling preserves. (b) **A pad that has stopped being TRIGGERED is still SOUNDING until its tail is out**, and `mixer`'s exclusive-idle early-out must not skip a row before then. `tail_frames` is an UPPER BOUND on that tail, not the tail itself: it is what the row's own lines hold, none of which resets or advances by itself, added up along the STRIP. Its applied insert-chain latency and its `pdc` are in series with everything after them, so they add; its `out_pdc` and its largest send-edge delay are PARALLEL branches off the same tap point (the sends leave before `out_pdc.process`), so only the longer of the two counts. Each of those lines exists only when its compensation is non-zero (`bus.rs:94` / `engine.rs:2023`), so a pad needing none carries none. What an insert or a live node holds BEYOND its declared latency — a reverb's decay, a synth's release — is not in the number. Skipping such a row both TRUNCATES what it still holds and replays it at the onset of the next press; naming the lines one at a time missed a plugin's own pipeline, which swallows a whole press. So the row keeps running the ORDINARY strip — minus its SOURCES (an off clock does not advance, so both the clip read and `render_live_into`'s scheduled-event queue would repeat themselves at the frozen position), with the fader gated on the tail rather than the clock — for `tail_frames` after its clock stops, and takes the bare skip only once that window is out. A row whose tail is 0 (every raw pad, and the measured 1.98 µs idle case) takes the bare skip on every block. Accepted consequence: an unbounded tail (a reverb on a pad) is hard-cut at the end of the window, exactly as a transport stop already cuts a track's inserts. |
