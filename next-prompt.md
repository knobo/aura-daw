...
2. **Plan F carry-forwards (undo)** — read the Plan F handoff in
   ...
   browser is in flight on `codex/undo-version-graph-ui`; do not duplicate it.
...
- **Plan F carry-forwards:** live-document B-tree, I-1 option (a), no journal auto-apply. The read-only version-graph browser is in flight on `codex/undo-version-graph-ui`. Read the Plan F handoff in `docs/PHASE4-PLAN.md` before touching snapshots, the journal reader, the version graph, or `engine::rebuild`. Branch `plan-f-history` is kept so cited SHAs resolve.
