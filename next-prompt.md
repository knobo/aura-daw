...
- **Plan F carry-forwards:** live-document B-tree, I-1 option (a), no journal auto-apply. The read-only version-graph browser is in flight as draft PR #82 on `codex/undo-version-graph-ui`; the ordered follow-up is the guarded linear `Undo to here` contract documented in the Plan F handoff. Read the Plan F handoff in `docs/PHASE4-PLAN.md` before touching snapshots, the journal reader, the version graph, or `engine::rebuild`. Branch `plan-f-history` is kept so cited SHAs resolve.
