 ...
 * the 0.8–2.0 clamp, the 0.1 step without float drift, and reset. The
 * canonical value lives in the preferences store (prefs.values.uiZoom).
 */

import { beforeEach, describe, expect, it } from "vitest";
import { prefs } from "../prefs/prefs.svelte";
import { UI_ZOOM_MAX, UI_ZOOM_MIN, resetUiZoom, setUiZoom, zoomUiIn, zoomUiOut } from "./ui.svelte";
...
describe("interface zoom store", () => {
  ...
  it("steps up by 0.1", () => {
    zoomUiIn();
    expect(prefs.values.uiZoom).toBe(1.1);
  });

  it("steps down by 0.1", () => {
    zoomUiOut();
    expect(prefs.values.uiZoom).toBe(0.9);
  });
  ...
  it("clamps at the maximum", () => {
    ...
    expect(prefs.values.uiZoom).toBe(UI_ZOOM_MAX);
  });

  it("clamps at the minimum", () => {
    for (let i = 0; i < 30; i++) zoomUiOut();
    expect(prefs.values.uiZoom).toBe(UI_ZOOM_MIN);
  });
  ...
  it("setUiZoom rounds to the 0.1 grid", () => {
    setUiZoom(1.2345);
    expect(prefs.values.uiZoom).toBe(1.2);
  });
