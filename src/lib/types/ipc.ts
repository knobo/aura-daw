...
export interface ProjectSnapshot {
   ...
   * start so the panel and the piano-roll tint have it before any composer
   * command runs. */
  harmony: HarmonyDoc;
}

// ── the Composer (Plan H1; harmony.schema.json / composer.schema.json) ─────
//
// Keys and chords are STRINGS on the wire ("C ionian", "Cmaj7"): the backend
// spells them from the line of fifths, and the frontend never parses or
// derives them (ADR 0006 — no theory in TypeScript).

export interface KeySpan {
  tick: number;
  /** Canonical key, e.g. "C ionian", "Bb aeolian". */
  key: string;
}
...
export interface ChordSpan {
  tick: number;
  lengthTicks: number;
  /** Chord symbol, e.g. "Cmaj7", "F#m7b5", "C/E". */
  chord: string;
}
...
/** What a pitch class is over the current chord — the five-tier colour ramp. */
...
export interface NoteClass {
  pitchClass: number;
  /** Best spelling in context ("Bb", not "A#"). */
  tpc: string;
  role: NoteRole;
  /** Degree label relative to the chord: "1", "b7", "#11". */
  degree: string;
  why?: string | null;
}

/** One of the twelve slots of the circle of fifths, spelled for the key. */
export interface CircleWedge {
  fifths: number;
  major: string;
  minor: string;
  inKey: boolean;
  chord?: string | null;
  roman?: string | null;
  borrowedChord?: string | null;
  function?: string | null;
  isTonic: boolean;
}

export interface HarmonySpanView {
  tick: number;
  lengthTicks: number;
  symbol: string;
  pretty: string;
...
export interface HarmonyNeighbour {
  key: string;
  label: string;
  steps: number;
  why: string;
...
export interface HarmonyBorrowed {
  symbol: string;
  pretty: string;
  roman: string;
  why: string;
...
export interface HarmonyView {
  ...
  neighbours: HarmonyNeighbour[];
  borrowed: HarmonyBorrowed[];
  schemas: ProgressionSchema[];
  genres: string[];
...
export interface ComposerSuggestion {
  ...
  roman: string;
  function: string;
  /** 0..1; the breakdown is in `why`. */
  score: number;
  why: string;
}

/** Why a generator did what it did — display only, never document state. */
...
export interface GeneratedClipInfo {
  part: string;
  clipId: string;
  trackId: string;
  trackName: string;
...
export interface ComposerGenerateRequest {
  ...
  parts?: string[];
  key?: string | null;
  /** A schema id, "circle", or "functional:<0-100>". */
  plan?: string | null;
  bars?: number | null;
  atTicks?: number | null;
  seed?: number | null;
  ...
  voicing?: string | null;
  rhythm?: string | null;
  bassStyle?: string | null;
  density?: number | null;
  syncopation?: number | null;
  trackIds?: Record<string, string>;
  writeHarmony?: boolean | null;
}

export interface ComposerGenerateReply {
  clips: GeneratedClipInfo[];
  harmony: HarmonyView;
  progression?: ProgressionView | null;
  /** The seed that produced this — stored so a take is recoverable. */
  seed: number;
  bars: number;
}
